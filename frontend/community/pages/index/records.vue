				...
				<view class="uni-list-cell" hover-class="uni-list-cell-hover" v-for="(item,index) in news" :key="index" @tap="openinfo" :data-newsid="item.post_id">
					<view class="uni-media-list">
						...
						<view class="uni-media-list-body">
							...
							<view class="uni-media-list-text-bottom uni-ellipsis">姓名：{{item.username}}</view>
							<view class="uni-media-list-text-bottom uni-ellipsis">联系电话：{{item.userTel}}</view>
							...
							<!-- <view class="uni-media-list-text-bottom uni-ellipsis">体温：{{item.userTel}}</view> -->
							<!-- 个人页面不用显示联系电话 -->
...
<script>
	export default {
		...
		onLoad:function(){
			...
			//发送get请求
			uni.request({
				url: 'http://localhost:9090/access/queryall',
				method: 'GET',
				data: {
					//tel:this.usertel,
